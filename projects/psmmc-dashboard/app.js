...
(function () {
      ...
      if (di >= 0) {
        ...
        if (d) {
          ...
          if (xi >= 0) {
            ...
            var dIso = isoDate(d);
            if (xd && (!rec.lastD || dIso >= rec.lastD)) {
              rec.lastD = dIso;
              rec.lastExp = isoDate(xd);
              if (bi >= 0 && row[bi]) rec.lastBatch = String(row[bi]).trim();
            }
          }
  ...
  function expiryStats(batches, avg, stock, asOfIso, approx) {
    ...
    for (var i = 0; i < batches.length; i++) {
      ...
      var tMo = d ? Math.max(0, (d - asOf) / 86400000 / DAYS_PER_MONTH) : 0;
      if (avg > 0) {
  ...
  /* The ROADMAP-1 alert predicate: at least one unit would expire unused AND
     that loss shortens the flat coverage by a month or more. */
  function expiryRisk(r) {
    return !!(r.moved && r.expWaste >= 1 && r.cov != null && r.expCov != null && r.cov - r.expCov >= 1);
  }
  ...
  function buildRows(wd, st) {
    ...
    Object.keys(codes).forEach(function (code) {
      ...
      // Expiry: real batches from the stock file win; otherwise the latest
      // dispatch batch from the withdrawals file approximates the queue front.
      var es = null;
  ...
  function buildReportWorkbook() {
    ...
    function batchSheet(kind) {
      var head = [t("c_code"), t("c_desc"), t("c_lot"), t("c_expdate"), t("ev_tte"), t("ev_total_qty"), t("c_planner"), t("c_value")];
      var aoa = [head];
      var asOf = parseIsoLocal(STATE.meta.stock_as_of) || new Date();
      STATE.rows.forEach(function (r) {
        ...
        list.forEach(function (b) {
          ...
          aoa.push([r.code, r.desc, b.b || "", prettyDate(b.e), mo, Math.round(b.q), plannerName(r) || t("planner_unassigned"), ""]);
        });
      });
      return sheetFrom(aoa, [16, 34, 16, 14, 11, 13, 18, 14], { 4: DEC1_FMT, 5: INT_FMT, 7: SAR_FMT }, true);
    }
