...
(function () {
  ...
  function buildDigest(rows, prev) {
    if (!prev) { STATE.digest = null; return; }
    var prevStatus = prev.statusByCode || null;
    var d = { danger: [], spikes: [], fresh: [], recovered: [] };
    rows.forEach(function (r) {
    ...
    STATE.digest = (d.danger.length || d.spikes.length || d.fresh.length || d.recovered.length) ? d : null;
  }
  ...
  function buildTableHTML(view, base) {
    ...
    if (view === "management") {
      ...
      body = rows.map(function (r) { return '<tr data-code="' + esc(r.code) + '">' + codeCell(r) + descCell(r) + "<td>" + esc(r.uom || "—") + "</td><td class=\"right num\">" + fmtInt(r.total) + "</td><td class=\"right num\">" + fmt1(r.avg) + "</td><td>" + trendCell(r) + "</td><td class=\"right num\">" + fmtInt(r.stock) + "</td><td>" + covCell(r) + "</td><td>" + statusCell(r) + "</td><td class=\"right num\">" + fmtInt(r.qty9) + "</td><td class=\"right num sug\">" + fmtInt(r.sug) + (r.seasonal ? ' <i class="seasonal-tag" title="' + esc(tFmt("ss_basis", { n: fmtInt(r.seasonal) })) + '">' + t("ss_tag") + "</i>" : "") + "</td></tr>"; }).join("");
    }
  ...
  function renderDetail(code) {
    ...
    return '<div class="dt-head"><span class="tile tile-lav tile-lg">' + ICON.pulse + '</span>'
      ...
      + catNote + stats + ooBlock + poBlock + priceBlock + chart + callouts + note + diBlock + thBlock;
  }
