...
(function () {
  ...
  function applyMap(rows) {
    ...
    rows.forEach(function (r) {
      ...
      r.alt = m ? [m.trade, m.sci, m.hosp, m.msd, m.cls].filter(Boolean).join(" ") : "";
      if (m) n++;
    });
    return n;
  }
  ...
  function buildRows(wd, st) {
    ...
    Object.keys(codes).forEach(function (code) {
      ...
      rows.push({ code: code, desc: (w && w.desc) || (s && s.desc) || "", alt: "", uom: (w && w.uom) || "", total: total, avg: avg, stock: stock, cov: cov, qty9: avg * ORDER_COVER_MONTHS, sug: Math.max(0, avg * ORDER_COVER_MONTHS - stock), status: statusOf(cov == null ? 0 : cov, avg, inStock), inStock: inStock, moved: avg > 0, trend: null, trendPct: null });
    });
    return rows;
  }
  ...
  function renderDetail(code) {
    ...
    var clsRow = (r.cls || r.prio)
      ? '<div class="dt-codes">' + (r.cls ? '<span class="callout lo">' + esc(r.cls) + '<i>' + t("dt_class") + '</i></span>' : "") + (r.prio ? '<span class="callout ' + (/LIFE/i.test(r.prio) ? "hi" : "lo") + '">' + esc(r.prio) + '<i>' + t("dt_priority") + '</i></span>' : "") + '</div>'
      : "";
    ...
    return '<div class="dt-head"><span class="tile tile-lav tile-lg">' + ICON.pulse + '</span>'
      + '<span class="ktxt"><div class="dt-title">' + esc(r.desc) + (r.trade ? ' <i class="tradename">' + esc(r.trade) + '</i>' : '') + '</div>'
